...
#######################################################################################################################################
# Part1. Making shapefiles based on csv files, project the shapefile to NDA 83 Albers, remove occurrences with NA in environmental 
...
# Load GBIF data for Zamia prasina
Sys.time()
occurrence <- fread("D:/xinchen/mee_sdm_experiment/0_downloaded_data/7_GBIF_data/0054373-240626123714530/occurrence.txt",
...
# Source: https://www.gbif.org/species/5284156
# Two names used in the occurrence on GBIF which are prasina and polymorpha, the filtered ratio of prasina to polymorpha is close to that on GBIF without any filtering

# Create a column with checked GBIF name
occurrence$gbif_name <- "Zamia_prasina"

# Load dissolved IUCN range maps
IUCN_all <- read_sf("D:/xinchen/mee_sdm_experiment/0_downloaded_data/4_processed_range_maps/Zamia_prasina_NAD83_Albers.shp")

# Load new matching table for connecting GBIF name with sci_name in IUCN maps
IUCN_species_list_9_matching <- data.frame(sci_name = "Zamia prasina",
                                           gbif_name = "Zamia_prasina")
...
occ_output <- "D:/xinchen/mee_sdm_experiment/5_R_script/Output/Zamia_prasina/occurrence/"
bg_output <- "D:/xinchen/mee_sdm_experiment/5_R_script/Output/Zamia_prasina/bg"
cali_area <- "D:/xinchen/mee_sdm_experiment/5_R_script/Output/Zamia_prasina/calibration_area/"
  ...
  if (nrow(temp_occ) > 0){
    ...
    if (nrow(temp_occ_in_1.1_buffer) != 0){
      ...
      if (nrow(temp_occ) > 1) {
        ...
        r_10km_i <- extend(r_10km_i, extent(r_10km_i) + 10000*10)
      ...
      if (nrow(temp_occ) >= 10) {
        ...
        # set the resolution of the cells to 1km
        # Date: 8/13/2024
        # Change: Change the size of thinning grid based on https://www.ngdc.noaa.gov/mgg/topo/report/s6/s6A.html since the study area is between 15 to 20 N
        res(r_10km_i) <- 872
        
        # expand (extend) the extent of the RasterLayer a little
        r_10km_i <- extend(r_10km_i, extent(r_10km_i) + 872)
